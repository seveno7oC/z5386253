...
def main(tic, update_csv=True):
    ...
    update_csv : bool
        If True, data_new will be downloaded. Defaults to True.
    ...
    Step 1: download.get_data(tic)
      Download stock price and recommendation data_new for a given `tic` and create
    ...
    Step 3: mk_events.mk_event_df(tic) --> dataframe
      Process the recommendations in `<rec csv>` and create a data_new frame with the
      events of interest.
    ...
    """
    # Step 1: Download stock price and recommendation data_new for `tic`
    if update_csv is True:
        download.get_data(tic)
    else:
        print("Parameter `update_csv` set to False, skipping downloads...")

    # Step 2: Create a data_new frame with stock (tic) and market returns
    ret_df = mk_rets.mk_ret_df(tic)

    # Step 3: Create a data_new frame with the events
