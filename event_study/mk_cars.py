...
def mk_cars_df(ret_df, event_df):
    """ Given a data_new frame with all events of interest for a given ticker
    (`event_df`) and the corresponding data_new frame with stock and market
    returns (`ret_df`), calculate the Cumulative Abnormal Return over the
    two-day window surrounding each event.

    Parameters
    ----------
    ret_df : pandas data_new frame
    ...
    event_df : pandas data_new frame
        Dataframe created by the function `mk_events.mk_event_df`. This data_new
...
def expand_dates(ser, window=2):
    """ For a given row in the data_new frame produced by the `mk_event_df`
    function above, return a data_new frame with the dates for the `window` days
    ...
    For instance, suppose window = 2 and consider the following row (an event):
     ...
     This function would produce the following data_new:
     ...
     which should be stored in a data_new frame with the following characteristics:
    ...
    # Create a new data_new frame with copies of the single-row data_new frame
...
def calc_car(ser, ret_df, window=2):
    """ For a given row in the data_new frame produced by the `mk_event_df` function
...
def _test_mk_cars_df(sample_only=False):
    ...
    # Create the `ret_df` and the `event_df` data_new frames
    tic = 'TSLA'
