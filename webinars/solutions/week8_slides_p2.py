...
QAN_PRC_CSV0 = os.path.join(cfg.DATADIR, 'qan_prc_2020_v2.csv')

# Reading the content of a file
df0 = pd.read_csv(QAN_PRC_CSV, index_col='Date', parse_dates=['Date'])
#utils.pprint(df0, "df0")
#df0.to_csv(QAN_PRC_CSV0, header=False)
...
#utils.pprint(df, "This is df (after)")

#df.set_index('date', inplace=True)
#utils.pprint(df, "This is df (after)")

#df1 = utils.csv_to_df(csv_cnts)
#utils.pprint(df1)
...
dt_index  = pd.to_datetime(date_ary)
...
df_2020_11_0204  = df.iloc['2020-11-02']
...
# (problem)
rets  = df.iloc[:, 'close'].pct_change(freq='B')
utils.pprint(rets)
