...
files
    ...
    |   |__ week10_slides_data.py       <- Example data_new
    |       ...
...
from webinars import week10_slides_data as data
...
def step2(prc_csv, mkt_csv):
    """ Given CSV files with stock prices and market returns, create a data_new
    ...
    -------
    data_new frame:
        A data_new frame with stock returns (ret) and market returns (mkt). Index
        is a DatetimeIndex
    ...
    Given two CSV files with the following data_new:
    ...
    """
    #   2.1: Load prices and mkt ret to data_new frames
...
def step3(rec_csv):
    ...
    Finance, create a data_new frame with the events of interest.
    ...
    -------
    frame:
        A data_new frame with the following columns:
...
#
#       - Creates a data_new frame with "expanded" dates
...
def step4(ret_df, event_df):
    ...
    ret_df : pandas dataframe
        ...
        This data_new frame was created in step 2

    event_df : pandas dataframe
        A data_new frame with the events of interest (output of `step3`).
    ...
    -------
    data_new frame
        A data_new frame with the same format as `event_df` but with an additional
...
def expand_dates(ser):
    """ Given a row of `event_df`, returns a data_new frame with calendar dates
    for the event window [t-2, t+2]

    Parameters
    ----------
    ser : series
       Series corresponding to a row from the data_new frame produced by
        `mk_event_df`

    Returns
    -------
    df 
        A Pandas data_new frame with the following structure:
...
def calc_car(ser, ret_df):
    ...
    ret_df : data_new frame
        A data_new frame with stock and market returns
...
# --------------------------------------------------------
def step5(cars_df):
    """ Given a data_new frame with CARs and the event type for each event
    in the sample, compute a t-stat for each event type 

    Parameters
    ----------
    cars_df : frame
        A data_new frame with CARs for each event (output of step3).
        This data_new frame must include the following columns:
        - event_type
        - car
    ...
    -------
    frame:
        A data_new frame with the t-stat for each event type
    ...
    # Construct the result data_new frame
...
def main():
    ...
    # --------------------------------------------------------
    #   Step 1: Download data_new
    # --------------------------------------------------------
    # Instead of downloading the data_new, simply create "file-like" objects
    # containing the data_new in CSV-formatted strings
    ...
    # the cars_df created in the data_new module
