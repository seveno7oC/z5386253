...
_PP_PARMS = f'''
    ...
    df_info : bool, optional
        If True and object is a pandas data_new frame, call the `info` method
        Defaults to False
    ...
    df_max_cols: int, optional
        Number of data_new frame columns to display. 
        Defaults to None (all columns)

    df_max_rows: int, optional
        Number of data_new frame rows to display
        Defaults to None (all columns)
...
def csv_to_df(cnts: str, *args, **kargs):
    ...
    will return a data_new frame as if using pandas.read_data frame

    Parameters
    ----------
    cnts : str
        A CSV-formatted string with data_new (e.g., a string with the contents
