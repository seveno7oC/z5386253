...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    file_dat = os.path.join(DATDIR, f"{tic}.dat")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    """Returns a dictionary containing the data_new for the tickers specified in tickers_lst.
        An Exception is raised if any of the tickers provided in tickers_lst or any of the
        column names provided in col_lst are invali d.
        ...
        -------
        dict
            A dictionary with format {<tic> : <data_new>} where
            - Each key (<tic>) is a ticker in tickers_lst (as a string)
            - Each value (<data_new>) is a dictionary with format
                {
                    'exchange': <tic_exchange>,
                    'data_new': [<dict_0>, <dict_1>, ..., <dict_n>]
                }
              where
        ...
        - If tickers_lst is None, the dictionary returned should contain the data_new for
    ...
    data[tic] = {'exchange': exchange,'data_new': data}
    return data
...
def create_json(data_dict, pth):
    """Saves the data_new found in the data_dict dictionary into a
...
def _test_create_data_dict():
    ...
    - Call `create_data_dict` using
        ...
        - col_lst = ['Date', 'Close']
    - Print out the dictionary returned, but only the first 3 items of the data_new list for each ticker for brevity
    ...
    for tic in tickers_lst:
        data_dict[tic]['data_new'] = data_dict[tic]['data_new'][:3]
...
if __name__ == "__main__":
    ...
    # _test_create_json(os.path.join(DATDIR, 'data_new.json'))  # Save the file to data_new/data_new.json
    pass
